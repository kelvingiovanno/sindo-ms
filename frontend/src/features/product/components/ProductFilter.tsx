...
import { brandOptions, modelOptions, categoryOptions, statusOptions } from "../dummy";
...
const ProductFilterMenu = () => {
    ...
    }, [selectedBrand, selectedModel, selectedCategories, selectedStatus, setSearchParams, searchParams]);
    ...
    const filter = data().filter((item) => item.label.toLocaleLowerCase().includes(search.toLocaleLowerCase()));
    ...
    const filterCount = () => {
        return selectedBrand.length + selectedModel.length + selectedCategories.length + selectedStatus.length;
    }
    ...
    return (
        <DropdownMenu>
            ...
            <DropdownMenuContent asChild className="p-0 ring-slate-300 rounded-xs">
